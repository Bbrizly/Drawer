...
final class HoverScrollMoverTests: XCTestCase {
    ...
    func testWarpTargetFlipsYAndAddsDelta() {
        ...
        XCTAssertEqual(t.y, 900 - 205, accuracy: 1e-6)
    }

    func testCGPointAppliesAppKitDelta() {
        let t = HoverScrollMover.cgPoint(
            byApplyingAppKitDeltaX: 10, deltaY: 5, to: CGPoint(x: 100, y: 200)
        )
        XCTAssertEqual(t.x, 110, accuracy: 1e-6)
        XCTAssertEqual(t.y, 195, accuracy: 1e-6)
    }
}
